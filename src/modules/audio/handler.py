...
class EventHandler:
    ...
    async def voice_state_update(self, event: VoiceStateUpdate):
        async def check_voice(bot, event: VoiceStateUpdate):
            ...
            if not (
                (prev_state and prev_state.channel_id == bot_state.channel_id)
                or (cur_state and cur_state.channel_id == bot_state.channel_id)
            ):
                return

            states = bot.cache.get_voice_states_view_for_guild(
                cur_state.guild_id
            ).items()
            user_count = len(
                [
                    state[0]
                    for state in filter(
                        lambda x: x[1].channel_id == bot_state.channel_id, states
                    )
                ]
