...
async def stats(ctx: lightbulb.Context) -> None:
    ...
    if (
        node_manager.nodes
        and (stats := node_manager.nodes[0].stats)
        and not stats.is_fake
    ):
        body += "Uptime: `{}`\nPlayers: `{} ({} playing)`\nMemory: `{} MB ({}%)`\n \
            Lavalink load: `{}%`\nSystem load: `{}%`\nFrames sent: `{}`\n".format(
            ...
            round(stats.memory_used / 1e6),
            round(100 * stats.memory_used / stats.memory_allocated),
            round(stats.lavalink_load * 100, 2),
            round(stats.system_load * 100, 2),
            stats.frames_sent,
        )
    else:
        body += "No stats available" + "\n"

    await ctx.respond(embed=hikari.Embed(title="📊 Lavalink Stats", description=body))
...
async def info(ctx: lightbulb.Context) -> None:
    ...
    await ctx.respond(embed=hikari.Embed(title="📊 Lavalink Info", description=body))
