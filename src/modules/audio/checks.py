...
def valid_user_voice(ctx: lightbulb.Context) -> bool:
    ...
    user_voice_state = next(
        filter(lambda i: i[0] == ctx.author.id, states.items()), None
    )
    bot_voice_state = next(
        filter(lambda i: i[0] == ctx.app.get_me().id, states.items()), None
    )
    ...
    if (
        bot_voice_state
        and user_voice_state[1].channel_id != bot_voice_state[1].channel_id
    ):
