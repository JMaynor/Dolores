...
def play_checks_options(
    func: lightbulb.decorators.CommandCallbackT,
) -> lightbulb.decorators.CommandCallbackT:
    func = lightbulb.add_checks(lightbulb.guild_only, valid_user_voice)(func)
    func = lightbulb.option(
        "loop", "Loop track/playlist", choices=["True"], default="False"
    )(func)
    func = lightbulb.option(
        "next", "Play track next", choices=["True"], default="False"
    )(func)
    func = lightbulb.option(
        "shuffle", "Disable playlist shuffle", choices=["False"], default="True"
    )(func)
    return func
...
async def get_choices(
    ...
    if source == YouTube:
        ...
        return [
            AutocompleteChoice(
                "🎬 {} [{}]".format(trim(track.title, 60), trim(track.author, 20)),
                track.uri,
            )
...
async def query_autocomplete(option, interaction):
    ...
    type_option = next(
        filter(lambda opt: opt.name == "type", interaction.options), None
    )
...
@lightbulb.option(
    "source",
    "Source to look up query",
    choices=[source.display_name for source in SOURCES],
    default=YouTube,
)
@lightbulb.option("type", "Type of query", choices=QUERY_TYPES, default=None)
@lightbulb.option(
    "query", "Query to search for.", required=True, autocomplete=query_autocomplete
)
