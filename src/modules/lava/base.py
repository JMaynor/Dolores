...
async def _play(
    ...
    if result.load_type == LoadType.PLAYLIST:
        ...
        if not result.plugin_info or result.plugin_info.get("type") not in (
            "artist",
            "playlist",
            "album",
        ):
            ...
            if result_type == "artist":
                playlist_url, image_url = (
                    plugin_info.get("url"),
                    plugin_info.get("artworkUrl"),
                )
                description = "[{}]({}) - `{} tracks`\n\n<@{}>".format(
                    plugin_info.get("author").upper(),
                    playlist_url,
                    num_tracks,
                    author_id,
                )
    ...
    return hikari.Embed(
        title=f"{result_type.capitalize()} added", description=description
